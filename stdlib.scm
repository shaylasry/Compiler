(define map
 ...
 (define fold-left 
	(let ((car car) (cdr cdr) (null? null?))
		(letrec ((fold-func (lambda (f acc list)
							(if (null? list)
								acc
								(fold-func f (f acc (car list)) (cdr list))))))
		(lambda (f acc list) (fold-func f acc list)))
	))
...
(define fold-right
	(let ((car car) (cdr cdr) (null? null?))
		(letrec ((fold-func (lambda (f acc list)
							(if (null? list)
								acc
								(f (car list) (fold-func f acc (cdr list)))))))
		(lambda (f acc list) (fold-func f acc list)))
	))
...
(define equal?
 ...
 (define cons*
  (let ((car car) (cdr cdr) (null? null?) (list? list?) (cons cons)  (eq? eq?))
    (letrec ((get-last (lambda (objects) (if (null? (cdr objects))
            									(car objects)
            									(get-last (cdr objects)))))
      		(make-list (lambda (objects final) (if (eq? (car objects) final)
              												final
															(cons (car objects) 
																  (make-list (cdr objects) final)))))
      		(cons-star (lambda (first-obj . objects) (if(null? objects)
            											first-obj
            											(make-list (cons first-obj objects) (get-last objects))))))
    cons-star)))